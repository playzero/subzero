...
fn ten() -> IdentityInfo {
	IdentityInfo {
		...
		discord: Data::Raw(b"The Right Ordinal Ten, Esq.".to_vec()),
		.. Default::default()
	}
}
...
fn twenty() -> IdentityInfo {
	IdentityInfo {
		...
		discord: Data::Raw(b"The Right Ordinal Twenty, Esq.".to_vec()),
		.. Default::default()
	}
}
...
fn adding_registrar_should_work() {
	new_test_ext().execute_with(|| {
		...
		let fields = IdentityFields(IdentityField::Display | IdentityField::discord);
