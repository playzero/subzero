...
// --> WeightInfo changed to private it seems
...
pub fn get_all_module_accounts() -> Vec<AccountId> {
	vec![
		// crowdfunding module account
		CrowdfundingModuleId::get().into_account(),
		// nft module account
		// NftModuleId::get().into_account(),
		// root account
		// ZeroAccountId::get(),
	]
}
...
impl module_crowdfunding::Config for Runtime {
	...
	type Currency = Balances;
	type Event = Event;
	type ModuleId = CrowdfundingModuleId;
...
construct_runtime!(
		...
		UncheckedExtrinsic = UncheckedExtrinsic
	{
		...
		// ZeroNFT: module_nft::{Module, Call, Storage, Event<T>},
